...
<?php
  $s      = (int)($step ?? 4); // step aktif: Itemset 3
    ...
    <div class="col-12 col-lg-11 col-xxl-10 page-wrap">
      ...
      <div class="card shadow-sm rounded-3"> 
        <div class="card-header py-3">
          <div class="m-0 fw-semibold">Frequent 3-Itemset</div> 
        </div>
        ...
        <div class="card-body">
          <div id="loadState" class="small text-muted mb-2">Memuat itemset 3…</div>
          <div id="errorState" class="alert alert-danger d-none"></div>

          <div class="table-responsive">
            <table class="table table-bordered table-sm table-zebra" id="tblItemset3">
              <thead>
                <tr>
          ...
          <div class="d-flex justify-content-between mt-3">
            <a class="btn btn-secondary" href="<?= site_url('report/itemset2/'.$reportId) ?>">Sebelumnya</a>
            <a class="btn btn-primary<?= empty($reportId) ? ' disabled' : '' ?>" href="<?= site_url('report/association-rule/'.$reportId) ?>">Selanjutnya</a>
          </div>
        </div>
      </div>
...
<script>
  ...
  const els = {
    ...
    tbody: document.querySelector('#tblItemset3 tbody'),
  ...
  async function loadItemset3(){
    ...
    try{
      const r = await fetch(`${apiBase}/itemset3/${encodeURIComponent(id)}`, { headers:{'Accept':'application/json'} });
      ...
      if (!Array.isArray(rows) || rows.length === 0){
        els.tbody.innerHTML = `<tr><td colspan="3" class="text-center text-muted">Tidak ada data itemset 3.</td></tr>`;
      } else {
        rows.forEach(row => {
      ...
      els.load.classList.add('d-none');
      els.err.textContent = 'Gagal memuat Itemset 3. ' + e.message;
      els.err.classList.remove('d-none');
      console.error(e);
    }
  }

  document.addEventListener('DOMContentLoaded', loadItemset3);
</script>
